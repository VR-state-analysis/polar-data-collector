...
def setup():
    cur = conn.cursor()
    cur.execute('''CREATE TABLE IF NOT EXISTS polar_data (
...
def add_entry(HeartRateMeasurement):
    cur = conn.cursor()
    rr_intervals = ' '.join(map(str, HeartRateMeasurement.rr_interval)) if HeartRateMeasurement.rr_interval else None
    cur.execute('''INSERT INTO polar_data (time, heart_rate, rr_intervals) 
                   VALUES (datetime('now'), ?, ?)''', 
